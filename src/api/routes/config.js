...
export default function createConfigRoutes(server) {
  ...
  router.get('/parameter-ranges', (req, res) => {
    res.json({
      ...
      entryDelayMinMs: { min: 1000, max: 5000, step: 100, label: '최소 진입 지연 (ms)', description: '반등 확인 후 최소 재확인 대기 시간', category: 'Scalping' },
      entryDelayMaxMs: { min: 1000, max: 5000, step: 100, label: '최대 진입 지연 (ms)', description: '반등 확인 후 최대 재확인 대기 시간', category: 'Scalping' },
