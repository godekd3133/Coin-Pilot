...
test('dashboard mock은 생성자 단계부터 사용자 persistence 경로를 격리한다', () => {
  ...
  try {
    ...
    assert.equal(path.dirname(trader.portfolioHistoryFile), os.tmpdir());
    ...
    assert.notEqual(path.resolve(trader.portfolioHistoryFile), path.resolve('portfolio_history.json'));
